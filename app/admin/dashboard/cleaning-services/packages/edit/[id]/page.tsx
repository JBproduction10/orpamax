...
const EditCleaningPackage = () => {
  const router = useRouter()
  const params = useParams()
  const id = params?.id as string
  const [formData, setFormData] = useState<CleaningPackage | null>(null)

  useEffect(() => {
    fetch(`/api/admin/cleaning/package/${id}`)
      .then(res => {
        if (!res.ok) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault()
    await fetch(`/api/admin/cleaning/package/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(formData),
    })

    router.push('/admin/dashboard/cleaning-services/packages')
  }
      ...
      <Card className="shadow-xl border border-gray-200">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <CardFooter className="flex justify-end">
            <Button type="submit" className="w-full md:w-auto">Update Package</Button>
          </CardFooter>
        </form>
      </Card>
    </div>
...
export default EditCleaningPackage
